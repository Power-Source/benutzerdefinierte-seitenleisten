...
(function($){
	window.csSidebars = {
		...
		/**
		 * This is the same prefix as defined in class-ps-sidebars.php
		 * @type string
		 */
		sidebar_prefix: 'cs-',
			...
			function handle_done_save( resp, okay, xhr ) {
				...
				if ( okay ) {
					if ( 'update' === resp.action ) {
						...
						$('.cs-wrap .ps-sidebars-add-new').detach();
					}
				} else {
					msg.type = 'err';
				}
		...
		showLocations: function( sb ){
			...
			function _add_new_rule( data, table ) {
				var template = wp.template('ps-sidebars-new-rule-row');
...
(function($){
    jQuery(document).ready( function($) {
        $('#screen-options-wrap .cs-allow-author input[type=checkbox]').on( 'change', function() {
            var data = {
                'action': 'benutzerdefinierte_seitenleisten_allow_author',
                '_wpnonce': $('#benutzerdefinierte_seitenleisten_allow_author').val(),
                'value': this.checked
            };
            $.post( ajaxurl, data );
        });
...
(function($){
    jQuery(document).ready( function($) {
        $('#screen-options-wrap .cs-custom-taxonomies input[type=checkbox]').on( 'change', function() {
            var data = {
                'action': 'benutzerdefinierte_seitenleisten_metabox_custom_taxonomies',
                '_wpnonce': $('#benutzerdefinierte_seitenleisten_custom_taxonomies').val(),
                'fields': {}
            };
...
(function($){
    jQuery(document).ready( function($) {
        $('#screen-options-wrap .cs-roles input[type=checkbox]').on( 'change', function() {
            var data = {
                'action': 'benutzerdefinierte_seitenleisten_metabox_roles',
                '_wpnonce': $('#benutzerdefinierte_seitenleisten_metabox_roles').val(),
                'fields': {}
            };
