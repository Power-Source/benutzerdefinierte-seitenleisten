...
add_action( 'cs_init', array( 'BenutzerdefinierteSeitenleistenCloning', 'instance' ) );
 ...
 * @since  2.0
 */
class BenutzerdefinierteSeitenleistenCloning {
	...
	public static function instance() {
		...
		if ( null === $Inst ) {
			$Inst = new BenutzerdefinierteSeitenleistenCloning();
		}
	...
	protected function settings_for_group( $settings, $group ) {
		...
		function_exists( 'wp_debug' ) &&  wp_debug( 'class-ps-sidebars-cloning.php:162', 'FAILED' );
		return false;
	}
	...
	public function admin_widget_button( $widget, $return, $instance ) {
		...
		<?php if ( ! isset( $_POST['csb-clone-button'] ) && ( 'widgets' === get_current_screen()->id ) ) : ?>
			<a href="#" class="button csb-clone-button"><?php _e( 'Clone', 'ps-sidebars' ); ?></a>
		<?php else : ?>
	 ...
	 * ClassicPress after the widget_update_callback filter is executed.
	 *
