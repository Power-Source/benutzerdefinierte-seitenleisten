...
add_action( 'cs_init', array( 'BenutzerdefinierteSeitenleistenReplacer', 'instance' ) );
 ...
 * @since  2.0
 */
class BenutzerdefinierteSeitenleistenReplacer extends BenutzerdefinierteSeitenleisten {
	...
	public static function instance() {
		...
		if ( null === $Inst ) {
			$Inst = new BenutzerdefinierteSeitenleistenReplacer();
		}
	...
	private function __construct() {
		add_action( 'widgets_init', array( $this, 'register_benutzerdefinierte_seitenleisten' ) );
	...
	/**
	 * Tell ClassicPress about the custom sidebars.
	 */
	public function register_benutzerdefinierte_seitenleisten() {
		$sb = self::get_benutzerdefinierte_seitenleisten();
		$sb = BenutzerdefinierteSeitenleisten::sort_sidebars_by_name( $sb );
		foreach ( $sb as $sidebar ) {
			 ...
			 * @since  2.0
			 *
			 * @param  array $sidebar Options used by ClassicPress to display
			 *           the sidebar.
			 */
	 ...
	 * Sidebars are replaced by directly modifying the ClassicPress globals
	...
	public function replace_sidebars() {
		...
		$benutzerdefinierte_seitenleisten_explain = BenutzerdefinierteSeitenleistenExplain::instance();
		$expl = $benutzerdefinierte_seitenleisten_explain->do_explain();
		...
		/**
		 * Original sidebar configuration by ClassicPress:
	 ...
	 * If the custom sidebar does not exist then the ClassicPress/Post options are
	...
	public function get_empty_widget() {
		$widget = new BenutzerdefinierteSeitenleistenEmptyPlugin();
		return array(
			'name'        => 'CS Empty Widget',
			'id'          => 'csemptywidget',
			'callback'    => array( $widget, 'display_callback' ),
			'params'      => array( array( 'number' => 2 ) ),
			'classname'   => 'BenutzerdefinierteSeitenleistenEmptyPlugin',
