...
add_action( 'cs_init', array( 'BenutzerdefinierteSeitenleistenExport', 'instance' ) );
 ...
 * @since  2.0
 */
class BenutzerdefinierteSeitenleistenExport extends BenutzerdefinierteSeitenleisten {
	...
	public static function instance() {
		...
		if ( null === $Inst ) {
			$Inst = new BenutzerdefinierteSeitenleistenExport();
		}
	...
	private function get_export_data() {
		...
		$data['sidebars'] = self::get_benutzerdefinierte_seitenleisten();
	...
	private function download_export_file() {
		...
		if (
			...
			|| ! wp_verify_nonce( $_POST['_wpnonce'], 'ps-sidebars-export' )
		) {
			$req = (object) array(
				'status' => 'ERR',
			);
	...
	private function read_import_file( $req ) {
		...
		if (
			! isset( $_POST['_wpnonce'] )
			|| ! wp_verify_nonce( $_POST['_wpnonce'], 'ps-sidebars-import' )
		) {
			$req = (object) array(
				'status' => 'ERR',
			);
