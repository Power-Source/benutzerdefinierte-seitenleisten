...
add_action( 'cs_init', array( 'BenutzerdefinierteSeitenleistenEditor', 'instance' ) );

/**
 * Provides all the functionality for editing sidebars on the widgets page.
 */
class BenutzerdefinierteSeitenleistenEditor extends BenutzerdefinierteSeitenleisten {
	 ...
	 * @since 3.0.9
	 */
	private $metabox_roles_name = 'benutzerdefinierte_seitenleisten_metabox_roles';
	 ...
	 * @since 3.0.9
	 */
	private $custom_taxonomies_name = 'benutzerdefinierte_seitenleisten_custom_taxonomies';
	 ...
	 * @since 3.1.5
	 */
	private $allow_author_name = 'benutzerdefinierte_seitenleisten_allow_author';
	...
	public static function instance() {
		static $Inst = null;
		if ( null === $Inst ) {
			$Inst = new BenutzerdefinierteSeitenleistenEditor();
		}
		return $Inst;
	}
	...
	public function settings() {
		...
		add_action( 'wp_ajax_benutzerdefinierte_seitenleisten_metabox_roles', array( $this, 'update_benutzerdefinierte_seitenleisten_metabox_roles' ) );
		add_action( 'wp_ajax_benutzerdefinierte_seitenleisten_metabox_custom_taxonomies', array( $this, 'update_benutzerdefinierte_seitenleisten_metabox_custom_taxonomies' ) );
		add_action( 'wp_ajax_benutzerdefinierte_seitenleisten_allow_author', array( $this, 'update_benutzerdefinierte_seitenleisten_allow_author' ) );
		/**
		 * Check user privileges
		 */
		$user_can_save = $this->current_user_can_update_benutzerdefinierte_seitenleisten();
		if ( ! $user_can_save ) {
			return;
		}
	...
	public function handle_ajax( $action ) {
		...
		if ( ! current_user_can( self::$cap_required ) ) {
			$req = self::req_err(
				$req,
				__( 'Du hast hierfür keine Erlaubnis', 'ps-sidebars' )
			);
		} else {
			switch ( $action ) {
				 ...
				 */
					...
					|| ! wp_verify_nonce( $_POST['_wpnonce'], 'ps-sidebars-get' )
					) {
						$req = self::req_err(
							$req,
							__( 'Du hast hierfür keine Erlaubnis', 'ps-sidebars' )
						...
						if ( $user_id ) {
							$advance = get_user_option( 'ps-sidebars-editor-advance', $user_id );
	...
	private function save_item( $req, $data ) {
		...
		if (
			! isset( $data['_wpnonce'] )
			|| ! wp_verify_nonce( $data['_wpnonce'], 'ps-sidebars-edit-sidebar' )
		) {
			return self::req_err(
				$req,
				__( 'Du hast keine Berechtigung, diesen Vorgang auszuführen.', 'ps-sidebars' )
			);
		}
		$sidebars = self::get_benutzerdefinierte_seitenleisten();
		$sb_id = $req->id;
		$sb_desc = '';
		...
		if ( empty( $sb_name ) ) {
			return self::req_err(
				$req,
				__( 'Der Name der Seitenleiste darf nicht leer sein', 'ps-sidebars' )
			);
		}
		if ( empty( $sb_id ) ) {
			...
			if ( ! $sidebar ) {
				return self::req_err(
					$req,
					__( 'Die Seitenleiste existiert nicht', 'ps-sidebars' )
				);
			}
		}
		...
		if ( 'insert' == $action ) {
			$sidebars[] = $sidebar;
			$req->message = sprintf(
				__( 'Neue Seitenleiste <strong>%1$s</strong> erstellt', 'ps-sidebars' ),
			...
			foreach ( $sidebars as $ind => $item ) {
				if ( $item['id'] == $sb_id ) {
					$req->message = sprintf(
						__( 'Seitenleiste <strong>%1$s</strong> aktualisiert', 'ps-sidebars' ),
						esc_html( $sidebar['name'] )
					);
			...
			if ( ! $found ) {
				return self::req_err(
					$req,
					__( 'Die Seitenleiste wurde nicht gefunden', 'ps-sidebars' )
				);
			}
		}
		// Save the changes.
		...
		if ( $user_id ) {
			...
			update_user_option( $user_id, 'ps-sidebars-editor-advance', $advance );
		}
		return $req;
	}
	...
	private function delete_item( $req, $data ) {
		...
		if (
			! isset( $data['_wpnonce'] )
			|| ! wp_verify_nonce( $data['_wpnonce'], 'ps-sidebars-delete-sidebar' )
		) {
			return self::req_err(
				$req,
				__( 'Du hast keine Berechtigung, diesen Vorgang auszuführen.', 'ps-sidebars' )
		...
		foreach ( $sidebars as $ind => $item ) {
			if ( $item['id'] == $req->id ) {
				$found = true;
				$req->message = sprintf(
					__( 'Seitenleiste <strong>%1$s</strong> gelöscht', 'ps-sidebars' ),
	...
	private function get_location_data( $req ) {
		...
		$archive_type = array(
			'_blog' => __( 'Startseite', 'ps-sidebars' ),
			'_search' => __( 'Suchergebnisse', 'ps-sidebars' ),
			'_404' => __( 'Nicht gefunden (404)', 'ps-sidebars' ),
			'_authors' => __( 'Beliebiges Autorenarchiv', 'ps-sidebars' ),
			'_date' => __( 'Datumsarchiv', 'ps-sidebars' ),
		);
		/**
		 * taxonomies
		 *
		...
		foreach ( $raw_taxonomies['_builtin'] as $taxonomy ) {
			...
			switch ( $taxonomy->name ) {
				 ...
				 */
					$archive_type['_tags'] = sprintf( __( '%s Archiv', 'ps-sidebars' ), $taxonomy->labels->singular_name );
				break;
				case 'category':
					$archive_type[ '_'.$taxonomy->name ] = sprintf( __( '%s Archive', 'ps-sidebars' ), $taxonomy->labels->singular_name );
				break;
			}
		}
		foreach ( $raw_taxonomies['custom'] as $taxonomy ) {
			if ( in_array( $taxonomy->labels->singular_name, $default_taxonomies ) ) {
				$archive_type[ '_taxonomy_'.$taxonomy->name ] = sprintf( __( '%s Archive', 'ps-sidebars' ), ucfirst( $taxonomy->name ) );
			} else {
				$archive_type[ '_taxonomy_'.$taxonomy->name ] = sprintf( __( '%s Archiv', 'ps-sidebars' ), $taxonomy->labels->singular_name );
			}
		}
		/**
		...
		foreach ( $raw_posttype as $item ) {
			if ( $item->name == 'post' ) {
				$label = __( 'Beitragsindex', 'ps-sidebars' );
			} else {
				if ( ! $item->has_archive ) { continue; }
				$label = sprintf(
					__( '%1$s Archive', 'ps-sidebars' ),
		...
		foreach ( $raw_taxonomies['custom'] as $t ) {
			...
			if (
				...
				$name  = sprintf( __( '%s Archiv', 'ps-sidebars' ), $t->labels->singular_name );
				if ( in_array( $t->labels->singular_name, $default_taxonomies ) ) {
					$name = sprintf( __( '%s Archive', 'ps-sidebars' ), ucfirst( $taxonomy ) );
				}
		 ...
		 */
		return apply_filters( 'benutzerdefinierte_seitenleisten_get_location', $req, $defaults );
	}
	...
	private function set_location_data( $req ) {
		...
		if (
			! isset( $_POST['_wpnonce'] )
			|| ! wp_verify_nonce( $_POST['_wpnonce'], 'ps-sidebars-set-location' )
		) {
			return self::req_err(
				$req,
				__( 'Du hast keine Berechtigung, diesen Vorgang auszuführen.', 'ps-sidebars' )
		...
		$req->message = sprintf(
			__( 'Einstellungen für <strong>%1$s</strong> aktualisiert.', 'ps-sidebars' ),
			esc_html( $req->sidebar['name'] )
		);
		/**
		 ...
		 */
		$options = apply_filters( 'benutzerdefinierte_seitenleisten_set_location', $options, $req->id, $sidebars, $data );
		self::set_options( $options );
		return $req;
	}
	...
	public function add_meta_box() {
		...
		if ( $pt_obj->publicly_queryable || $pt_obj->public ) {
			add_meta_box(
				'benutzerdefinierteseitenleisten-mb',
				__( 'Seitenleisten', 'ps-sidebars' ),
	 ...
	 *
	 * @since  2.0.9.7
	 * @param  array $benutzerdefinierte_seitenleisten List of the custom sidebars.
	 */
	static protected function wpml_update( $benutzerdefinierte_seitenleisten ) {
		...
		foreach ( $benutzerdefinierte_seitenleisten as $fields ) {
	...
	public function post_columns( $columns ) {
		...
		$insert = array(
			'cs_replacement' => _x( 'Benutzerdefinierte Seitenleisten', 'Spaltenname in der Eintragsliste.', 'ps-sidebars' ),
		);
	...
	public function post_quick_edit_js() {
		...
		jQuery(function() {
			...
			inlineEditPost.edit = function( id ) {
				...
				// we don't want to leave ClassicPress hanging
	...
	public function bulk_edit_save( $post_id ) {
		if ( ! isset( $_REQUEST['ps-sidebars-editor-bulk-edit'] ) ) {
			return;
		}
		if ( ! wp_verify_nonce( $_REQUEST['ps-sidebars-editor-bulk-edit'], 'bulk-edit-cs' ) ) {
			return;
		}
		if ( null == $this->modifiable ) {
			$this->modifiable = BenutzerdefinierteSeitenleisten::get_options( 'modifiable' );
		}
		if ( empty( $this->modifiable ) ) {
			return;
		}
		$update = false;
		$data = BenutzerdefinierteSeitenleisten::get_post_meta( $post_id );
	...
	public function add_capabilities_select_box( $screen_settings, $screen ) {
		if ( 'widgets' == $screen->base && current_user_can( 'manage_options' ) ) {
			...
			$screen_settings .= sprintf( '<legend>%s</legend>', __( 'Die benutzerdefinierte Konfiguration der Seitenleisten ist zulässig für:', 'ps-sidebars' ) );
			...
			$screen_settings .= sprintf( '<legend>%s</legend>', __( 'Erlaubt, Seitenleisten zu ersetzen:', 'ps-sidebars' ) );
			$checked = get_option( $this->allow_author_name, false );
			$screen_settings .= sprintf(
				'<label><input type="checkbox" name="cs-allow-author" value="%s" %s /> %s</label>',
				esc_attr( $role ),
				checked( $checked, true, false ),
				esc_html__( 'Erlaube dem Autor des Eintrags, die Seitenleisten zu ändern', 'ps-sidebars' )
			);
			$screen_settings .= '</fieldset>';
			/**
			 * Custom taxonomies
			 */
			$taxonomies = BenutzerdefinierteSeitenleisten::get_custom_taxonomies();
			if ( ! empty( $taxonomies ) ) {
				$allowed = get_option( $this->custom_taxonomies_name, array() );
				...
				$screen_settings .= sprintf( '<legend>%s</legend>', __( 'Benutzerdefinierte Taxonomien in der Seitenleiste zulassen:', 'ps-sidebars' ) );
				...
				$screen_settings .= '</fieldset>';
				$screen_settings .= sprintf( '<p class="description">%s</p>', __( 'Nachdem Du eine benutzerdefinierte Taxonomie aktiviert hast, musst Du diesen Bildschirm neu laden, um ihn in der Seitenleiste auswählen zu können.', 'ps-sidebars' ) );
			}
		}
	 ...
	 * @since 3.0.9
	 */
	public function update_benutzerdefinierte_seitenleisten_metabox_roles() {
	 ...
	 * @since 3.1.4
	 */
	public function update_benutzerdefinierte_seitenleisten_metabox_custom_taxonomies() {
	 ...
	 * @since 3.1.5
	 */
	public function update_benutzerdefinierte_seitenleisten_allow_author() {
		/**
		 * check required data
		 */
	 ...
	 * @since 3.0.9
	 */
	public function current_user_can_update_benutzerdefinierte_seitenleisten() {
		$allowed = get_option( $this->metabox_roles_name, 'any' );
		if ( is_string( $allowed ) && 'any' == $allowed ) {
					return true;
		} else {
