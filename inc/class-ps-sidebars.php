...
require_once CSB_INC_DIR . 'class-ps-sidebars-widgets.php';
require_once CSB_INC_DIR . 'class-ps-sidebars-editor.php';
require_once CSB_INC_DIR . 'class-ps-sidebars-replacer.php';
require_once CSB_INC_DIR . 'class-ps-sidebars-cloning.php';
require_once CSB_INC_DIR . 'class-ps-sidebars-visibility.php';
require_once CSB_INC_DIR . 'class-ps-sidebars-export.php';
require_once CSB_INC_DIR . 'class-ps-sidebars-explain.php';

require_once CSB_INC_DIR . 'class-ps-sidebars-checkup-notification.php';


/**
 * Main plugin file.
 * The BenutzerdefinierteSeitenleisten class encapsulates all our plugin logic.
 */
class BenutzerdefinierteSeitenleisten {
	/**
	 ...
	 * @var  string
	 */
	static public $pro_url = 'https://github.com/Power-Source/piestingtal-source-project/ps-seitenleisten/';
	...
	static public function instance() {
		...
		if ( null === $Inst ) {
			$Inst = new BenutzerdefinierteSeitenleisten();
		}
	...
	public function admin_init() {
		...
		if ( in_array( 'wpmudcs1', $dismissed_wp_pointers ) || wp_is_mobile() ) {
			...
			lib3()->html->pointer(
				...
				sprintf(
					__(
						'Jetzt kannst Du benutzerdefinierte Seitenleisten in Deinem ' .
						'<a href="%1$s">Widgets Bildschirm</a> erstellen und bearbeiten!', 'ps-sidebars'
					),
		...
		if ( true === self::$accessibility_mode ) {
			...
			lib3()->ui->admin_message(
				sprintf(
					__(
						'<strong>Der Eingabehilfenmodus wird vom %1$s-Plugin nicht unterstützt.</strong> '.
						'<br /><a href="%2$s">Klicke hier</a> um den Eingabehilfenmodus ' .
						'zu deaktivieren und das %1$s-Plugin zu verwenden !',
						'ps-sidebars'
					),
					$plugin_title,
	 ...
	 */
	static public function get_benutzerdefinierte_seitenleisten() {
	 ...
	 * @since  2.0
	 */
	static public function set_benutzerdefinierte_seitenleisten( $value ) {
		// Permission check.
		if ( ! current_user_can( self::$cap_required ) ) {
			return;
		}
	 ...
	 * widgets (this is stored inside a ClassicPress core option).
	 *
	...
	/**
	 * Update the ClassicPress core settings for sidebar widgets:
	...
	static public function refresh_sidebar_widgets() {
		...
		$cs_sidebars = self::get_benutzerdefinierte_seitenleisten();
	...
	static public function get_sidebars( $type = 'theme' ) {
		...
		$allsidebars = BenutzerdefinierteSeitenleisten::sort_sidebars_by_name( $wp_registered_sidebars );
		$result = array();
	...
	public function add_action_links( $actions, $plugin_file, $plugin_data, $context ) {
		if ( current_user_can( 'edit_theme_options' ) ) {
			$actions['widgets'] = sprintf(
				...
				__( 'Widgets', 'ps-sidebars' )
			);
		}
		$url = 'https://wordpress.org/support/plugin/ps-sidebars';
		
		$actions['support'] = sprintf(
			'<a href="%s">%s</a>',
			esc_url( $url ),
			__( 'Support', 'ps-sidebars' )
		);
		return $actions;
	}
...
?>
	<script type="text/html" id="tmpl-ps-sidebars-new">
		<div class="ps-sidebars-add-new">
			<p><?php esc_html_e( 'Erstelle eine benutzerdefinierte Seitenleiste, um loszulegen.', 'ps-sidebars' ); ?></p>
		</div>
	</script>
    <script type="text/html" id="tmpl-ps-sidebars-new-rule-row">
        <tr>
            <td>
                <select name="cs-screen[minmax][]">
                    <option value="max"<# if( 'max' == data.minmax ) { #> selected="selected"<# } #>><?php esc_html_e( 'max', 'ps-sidebars' ); ?></option>
                    <option value="min"<# if( 'min' == data.minmax ) { #> ' selected="selected"<# } #>><?php esc_html_e( 'min', 'ps-sidebars' ); ?></option>
                </select>
            </td>
            <td>
                <select name="cs-screen[mode][]">
                    <option value="hide"<# if( 'hide' == data.mode ) { #> selected="selected"<# } #>><?php esc_html_e( 'Hide', 'ps-sidebars' ); ?></option>
                    <option value="show"<# if( 'show' == data.mode ) { #> selected="selected"<# } #>><?php esc_html_e( 'Show', 'ps-sidebars' ); ?></option>
                </select>
            </td>
...
<?php
	...
	public function integrations() {
		...
		if ( function_exists( 'icl_object_id' ) && ! defined( 'POLYLANG_VERSION' ) ) {
			require_once CSB_INC_DIR . 'integrations/class-ps-sidebars-integration-wpml.php';
		}
		/**
		 * 3rd party plugins integration: Polylang
		 */
		if ( defined( 'POLYLANG_VERSION' ) && POLYLANG_VERSION ) {
			require_once CSB_INC_DIR . 'integrations/class-ps-sidebars-integration-polylang.php';
		}
		/**
		 * 3rd party plugins integration: WP Multilang
		 */
		if ( defined( 'WPM_PLUGIN_FILE' ) && WPM_PLUGIN_FILE && file_exists( WPM_PLUGIN_FILE ) ) {
			require_once CSB_INC_DIR . 'integrations/class-ps-sidebars-integration-wml.php';
		}
	...
	public static function get_custom_taxonomies( $state = 'all' ) {
		 ...
		 */
		if ( 'allowed' === $state ) {
			$editor = BenutzerdefinierteSeitenleistenEditor::instance();
