...
add_action( 'cs_init', array( 'BenutzerdefinierteSeitenleistenExplain', 'instance' ) );
 ...
 *
 */
class BenutzerdefinierteSeitenleistenExplain extends BenutzerdefinierteSeitenleisten {
	...
	public static function instance() {
		...
		if ( null === $Inst ) {
			$Inst = new BenutzerdefinierteSeitenleistenExplain();
		}
	...
	private function __construct() {
		...
		if ( 0 == $this->current_user_id ) {
			$this->debug = apply_filters( 'benutzerdefinierte_seitenleisten_explain', $this->debug );
		} else {
			$this->debug = (boolean) get_user_meta( $this->current_user_id, 'benutzerdefinierte_seitenleisten_explain', true );
			$this->set_explain();
		}
	...
	public function set_explain() {
		...
		if ( current_user_can( 'manage_options' ) && 'on' == $_GET['cs-explain'] ) {
			$this->debug = true;
			$result = add_user_meta( $this->current_user_id, 'benutzerdefinierte_seitenleisten_explain', $this->debug, true );
			if ( ! $result ) {
				update_user_meta( $this->current_user_id, 'benutzerdefinierte_seitenleisten_explain', $this->debug );
			}
			return;
		}
		$this->debug = false;
		delete_user_meta( $this->current_user_id, 'benutzerdefinierte_seitenleisten_explain' );
	}
